...
describe('ODM Model multi-tenant', () => {
  ...
  beforeAll(async () => {
    const defaultConnection = await testingDB.connect();
  ...
  describe('when multitenant not active, should always use default db', () => {
    it('should throw an error', async () => {
      MultiTenantConfig.active = false;
      await tenants.run({ name: 'db1' }, async () => {
        expect(tenants.current()).toBe('default');
      });
    });
  });

  describe('when multitenant active and not tenant found on current async context', () => {
    it('should throw an error', async () => {
  ...
  describe('save', () => {
    it('should save to diferent dbs based on tenant on the context', async () => {
      ...
      await tenants.run({ name: 'db2' }, async () => {
        await testingModel.save({
          name: 'document 2',
        });
      });
